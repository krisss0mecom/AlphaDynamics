...
3. **Empirical observations.** We report two trends: the optimal ODE
   ...
   largest for conformationally ordered domains (§4.3). These
...
4. **Rollout stability.** Six long (2500-step) autoregressive rollouts
   ...
   time. We report this honestly as a current limitation (§4.4).
...
adjoint scheme (Chen et al. 2018). The natural frequencies $\omega_k$
and anchors $\alpha_k$ are initialized to break symmetry and avoid
resonances. The coupling matrix $W\in\mathbb{R}^{M\times M}$ is
learnable; the anchor amplitude $a$ and the scaling of $\omega_k$ are
learnable scalars.
...
that rollout fidelity does not visibly degrade (§4.4), not that the NLL
...
Completing the remaining historical N about 50 aligned rerun would strengthen
...
optimal for every domain. Replacing this heuristic with calibrated
multi-step training or another exposure-bias correction remains an open
problem.

*Cross-temperature.* Auxiliary checks showed cross-temperature wins across
320–450 K, though all headline aligned audit results train and evaluate at
348 K. We do not use cross-temperature results as a primary v1 claim.
...
All source code, training scripts, and per-domain result tables are available at
